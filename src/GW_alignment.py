...
class Create_initial_plans():
    ...
    def unif_plans(self):
        unif_mat = np.random.uniform(0, 1, (self.n_init_plan, self.n_column, self.n_column))
...
class GW_alignment():
    ...
    def _GW_1to1(self, trial):
        ...
        while gwd == 0 and n_iter < 3:
            ...
            for i in range(self.n_init_plan):
                ...
                gw,log = my_entropic_gromov_wasserstein(C1 = self.RDM_1, C2 = self.RDM_2, p = p, q = q, T = T, epsilon = epsilon, loss_fun="square_loss", verbose=False, log=True)
                gwd = log['gw_dist']
                gws.append(gw)
                gwds.append(gwd)
            idx = np.argmin(gwds)
    ...
    def optimize(self, n_trials, n_jobs, parallel = True):
        ...
        """
        # Create study
        study = optuna.create_study(study_name = self.study_name, storage = self.DATABASE_URL, load_if_exists = True)
        # Reset study
