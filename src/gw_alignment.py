...
class GW_Alignment():
    def __init__(self, pred_dist, target_dist, p, q, save_path, max_iter = 1000, n_iter = 100, to_types = 'torch'):
        ...
        self.main_compute = MainGromovWasserstainComputation(pred_dist, target_dist, p, q, self.to_types, max_iter = max_iter, n_iter = n_iter)
    ...
    def __call__(self, trial, device, init_plans_list, eps_list, eps_log=True):
        ...
        '''
        gw, logv, gw_loss, init_mat, trial = self.main_compute.compute_GW_with_init_plans(trial, eps, init_mat_plan, device)
